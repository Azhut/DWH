...
- [ ] Сохраняется мало `flat_data`, потому что null ячейки не сохраняются в flat_data, эта проблема есть для всех форм. -- снэпшот неправильынй для 1ФК. Проблема в FlatDataBuilder `app/domain/parsing/flat_data_builder.py`
